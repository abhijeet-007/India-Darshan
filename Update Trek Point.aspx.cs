...
public partial class Update_Trek_Point : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from TrekData", con);
    ...
    protected void UpdateButton_Click(object sender, EventArgs e)
    {
        ...
        if (FileUpload1.HasFile || FileUpload2.HasFile || FileUpload3.HasFile)
        {
            ...
            FileUpload1.SaveAs(Server.MapPath("~/Uploads/Trekpoints/") + Path.GetFileName(FileUpload1.FileName));
            FileUpload2.SaveAs(Server.MapPath("~/Uploads/Trekpoints/") + Path.GetFileName(FileUpload2.FileName));
            FileUpload3.SaveAs(Server.MapPath("~/Uploads/Trekpoints/") + Path.GetFileName(FileUpload3.FileName));
            String link = "Uploads/Trekpoints/" + Path.GetFileName(FileUpload1.FileName);
            String link2 = "Uploads/Trekpoints/" + Path.GetFileName(FileUpload2.FileName);
            String link3 = "Uploads/Trekpoints/" + Path.GetFileName(FileUpload3.FileName);

            string query = "update TrekData set TImage = ('" + link + "'), TImage1 = ('"+link2+"'), TImage2 = ('"+link3+"') where TName = ('" + SL + "')";
