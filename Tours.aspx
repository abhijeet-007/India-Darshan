<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage2.master" AutoEventWireup="true" CodeFile="Tours.aspx.cs" Inherits="Tours" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">

    
    <div class="bg-white">

    <div>
        <table style="width: 100%" >
            <tr style="height:35px">
                <td style="width: 175px; text-align: center;" align="center">
     <h4>&nbsp;<asp:Label ID="Label3" runat="server" ForeColor="#000066" 
             Text="Search State :-"></asp:Label>
                    </h4>
                </td>
                <td style="width: 224px; " align="center">
                    <asp:TextBox ID="searchfield" runat="server" BorderStyle="Solid" Width="218px" 
                        Height="25px"></asp:TextBox>
                </td>
                <td style="text-align: left">
        <asp:Button CssClass="btn" ID="Bsearch" runat="server" Text="Search" OnClick="Search" 
                        Width="145px"  Height="30px"/>

                </td>
            </tr>
        </table>

       

        <asp:DataList ID="DataList1" runat="server" DataKeyField="SName" 
             OnItemCommand="DataList1_ItemCommand" 
            RepeatColumns="1" RepeatDirection="Horizontal">
            <ItemTemplate>
                
                <table style="width: 100%; height: 129px">
                    <tr>
                        <td rowspan="2" style="width: 82px">
                            <%--<asp:Image ID="Image1" runat="server" Height="150px" 
                                ImageUrl='<%# Eval("SImage") %>' Width="250px" />--%>
                            <asp:ImageButton ID="ImageButton1" runat="server" Height="150px" Width="250px"
                                ImageUrl='<%# Eval("SImage") %>'  CommandName="Images" 
                                CommandArgument='<%# Eval("SName") %>' oncommand="ImageButton1_Command"/>
                        </td>
                        <td style="height: 5px">
                            <asp:Label ID="Label1" runat="server" Text='<%# Eval("SName") %>' 
                                Font-Size="Large" ForeColor="#000066"></asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td style="height: 65px">
                            <asp:Label ID="Label2" runat="server" Text='<%# Eval("SDescription") %>' 
                                ForeColor="#000066"></asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 82px">
                            <asp:Button ID="Button1" runat="server" CommandArgument='<%# Eval("SName") %>' 
                                CommandName="Placess" CssClass="btn" Text="View Places" Width="250px" />
                        </td>
                        <td>
                            &nbsp;</td>
                    </tr>
                </table>
               
            </ItemTemplate>
        </asp:DataList>

        <!--<asp:SqlDataSource ID="SqlDataSource2" runat="server" 
            ConnectionString="<%$ ConnectionStrings:RegistrationConnectionString %>" 
            SelectCommand="SELECT * FROM [StateData]"></asp:SqlDataSource>-->

    
    

</div>
</div>
    
    
   

    
    


</asp:Content>

